...
test('6D uses deterministic safe acquisition and computed referral projections',()=>{
 ...
 assert.ok(s.includes('member_referral_attributions'));assert.ok(s.includes('ORDER BY occurred_at ASC,id ASC'));assert.ok(s.includes('rows.at(-1)'));
...
test('6D projections do not serialize private acquisition or relationship identifiers',()=>{
 const s=fs.readFileSync(new URL('crm/acquisition.ts',root),'utf8');assert.ok(s.includes("return {hasReferrer:Boolean(row),referrerLabel:row?.label||null}"));assert.equal(s.includes('attributionId'),false);
});