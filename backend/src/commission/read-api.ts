...
const sourceDomain = (value: unknown): 'TRAVEL' | null => value === 'TRAVEL' ? 'TRAVEL' : null;
...
export async function commissionAttributionSnapshot(db: D1Database, input: { workspaceId: string; lineAccountId: string; days: number; programId?: string; dealerId?: string; now?: Date }) {
  ...
  const safeSourceDomain = "CASE WHEN c.conversion_source='TRAVEL' THEN 'TRAVEL' ELSE NULL END";
  const sourceRows: any[] = (await db.prepare(`SELECT a.attribution_source source,${safeSourceDomain} source_domain,COUNT(*) count${base}${where} GROUP BY a.attribution_source,${safeSourceDomain} ORDER BY source,source_domain`).bind(...args).all()).results || [];
  const recentRows: any[] = input.dealerId ? [] : (await db.prepare(`SELECT a.id attribution_id,a.program_id,p.name program_name,a.attribution_source,${safeSourceDomain} source_domain,c.occurred_at,a.attributed_at${base} JOIN commission_programs p ON p.id=a.program_id AND p.workspace_id=a.workspace_id AND p.line_account_id=a.line_account_id${where} ORDER BY c.occurred_at DESC,a.id DESC LIMIT 20`).bind(...args).all()).results || [];
  return {
    ...
    sources: sourceRows.map(row => ({ attributionSource: String(row.source), sourceDomain: sourceDomain(row.source_domain), attributedConversions: count(row.count) })),
    recent: recentRows.map(row => ({ attributionId: String(row.attribution_id), programId: String(row.program_id), programName: String(row.program_name), conversionCategory: 'CONVERSION', attributionSource: String(row.attribution_source), sourceDomain: sourceDomain(row.source_domain), occurredAt: row.occurred_at || null, attributedAt: row.attributed_at || null })),
  };
}
