...
import { conversionSourceHealthRows } from '../journey/conversion-sources.ts';
import { evaluateOptimizationDataQuality } from '../optimization/recommendations.ts';
...
export async function buildGuideContext(input: BuildGuideContextInput): Promise<GuideContext | null> {
  ...
  const jq:any=journeySummary||{}; const observedSessions=Number(jq.observed_sessions||0), observedActions=Number(jq.observed_actions||0), webhookRoutes=Number(jq.webhook_routes||0), conversions=Number(jq.conversions||0), journeyMetricsThrough=clean(jq.metrics_through); const journeyFreshness=journeyMetricsThrough?Math.floor((Date.now()-Date.parse(journeyMetricsThrough+'T00:00:00Z'))/86400000):Infinity; const journeyMappingRatio=Number(journeyMapping?.total||0)?Number(journeyMapping?.mapped||0)/Number(journeyMapping?.total||0):0; const integrationAvailable=Boolean(conversionKey?.id); const reasonCodes:string[]=[]; if(!observedActions)reasonCodes.push('NO_JOURNEY_DATA'); if(observedSessions<30)reasonCodes.push('INSUFFICIENT_OBSERVED_SESSIONS'); if(webhookRoutes<20)reasonCodes.push('INSUFFICIENT_WEBHOOK_SAMPLES'); if(integrationAvailable&&conversions<10)reasonCodes.push('INSUFFICIENT_CONVERSION_SAMPLES'); if(journeyFreshness>3)reasonCodes.push('STALE_DATA'); if(Number(journeyMapping?.total||0)>0&&journeyMappingRatio<.8)reasonCodes.push('MAPPING_INCOMPLETE'); if(!integrationAvailable)reasonCodes.push('NO_CONVERSION_INTEGRATION'); const journey:any={period:{from,to:today,days:30},project:{observedActions,aggregateClicks:clicks,keywordMatches:Number(jq.keyword_matches||0),webhookRoutes,webhookSuccesses:Number(jq.webhook_successes||0),webhookFailures:Number(jq.webhook_failures||0),conversions,conversionValueMinor:Number(jq.conversion_value_minor||0)},areas:(journeyAreas.results||[]).map((row:any)=>({areaId:clean(row.area_id),label:clean(row.label),actionType:clean(row.action_type),sessions:Number(row.sessions||0),observedActions:Number(row.observed_actions||0),keywordMatches:Number(row.keyword_matches||0),webhookSuccesses:Number(row.webhook_successes||0),conversions:Number(row.conversions||0),conversionValueMinor:Number(row.conversion_value_minor||0),aggregateClicks:Number(row.aggregate_clicks||0),observedConversionRate:Number(row.observed_actions||0)?Number(row.conversions||0)/Number(row.observed_actions||0):null})),dataQuality:{ready:reasonCodes.filter(x=>x!=='NO_CONVERSION_INTEGRATION').length===0,reasonCodes,observedSessions,webhookSamples:webhookRoutes,conversionSamples:conversions,freshnessDays:journeyFreshness,mappingRatio:journeyMappingRatio,conversionIntegrationAvailable:integrationAvailable}};  const [trackedResult, sourceRows] = await Promise.all([
    db.prepare(`SELECT project_area_id,COUNT(*) tracked_uri_clicks,SUM(CASE WHEN conversion_event_id IS NOT NULL THEN 1 ELSE 0 END) attributed_conversions FROM tracked_uri_attributions WHERE workspace_id=? AND project_id=? AND status='clicked' AND occurred_at>=? GROUP BY project_area_id`).bind(workspaceId,entityId,from+'T00:00:00.000Z').all<Record<string,unknown>>(),
    db.prepare(`SELECT conversion_source,COUNT(*) event_count,MAX(occurred_at) last_event_at,COUNT(*) conversions FROM line_conversion_events WHERE workspace_id=? GROUP BY conversion_source`).bind(workspaceId).all<Record<string,unknown>>(),
  ]);
  const trackedByArea = new Map((trackedResult.results || []).map((row:any) => [clean(row.project_area_id), row]));
  const sourceHealth = conversionSourceHealthRows((sourceRows.results || []) as any[], Date.now(), integrationAvailable);
  const optimizationAreas = journey.areas.map((item:any) => { const tracked:any=trackedByArea.get(item.areaId); const trackedUriClicks=Number(tracked?.tracked_uri_clicks||0), attributedConversions=Number(tracked?.attributed_conversions||0), aggregateLineClicks=Number(item.aggregateClicks||0); const trackingAvailable=item.actionType==='uri'; return { ...item, aggregateLineClicks, trackedUriClicks, attributedConversions, trackedObservedConversionRate:trackedUriClicks?attributedConversions/trackedUriClicks:null, trackingAvailable, trackingEnabled:trackingAvailable&&trackedUriClicks>0, attributionCoverage:aggregateLineClicks+trackedUriClicks>0?trackedUriClicks/(aggregateLineClicks+trackedUriClicks):null }; });
  const totalTrackedUriClicks=optimizationAreas.reduce((sum:number,item:any)=>sum+Number(item.trackedUriClicks||0),0), totalAttributedConversions=optimizationAreas.reduce((sum:number,item:any)=>sum+Number(item.attributedConversions||0),0), totalAggregateLineClicks=optimizationAreas.reduce((sum:number,item:any)=>sum+Number(item.aggregateLineClicks||0),0);
  const optimization:any={period:{from,to:today,days:30},project:{aggregateLineClicks:totalAggregateLineClicks||clicks,trackedUriClicks:totalTrackedUriClicks,observedActions,attributedConversions:totalAttributedConversions,attributionCoverage:totalAggregateLineClicks+totalTrackedUriClicks>0?totalTrackedUriClicks/(totalAggregateLineClicks+totalTrackedUriClicks):null,uriTrackingAvailable:areas.some(area=>area.actionType==='uri'),conversionIntegrationAvailable:integrationAvailable,conversionSourceEvents:(sourceRows.results||[]).reduce((sum:any,row:any)=>sum+Number(row.event_count||0),0),activeConversionSources:sourceHealth.filter((row:any)=>row.status==='ACTIVE').length,staleConversionSources:sourceHealth.filter((row:any)=>row.status==='STALE').length,webhookFailureRate:webhookRoutes?Number(jq.webhook_failures||0)/webhookRoutes:null,freshnessDays:Math.max(freshness,journeyFreshness)},areas:optimizationAreas,sourceHealth};
  optimization.dataQuality=evaluateOptimizationDataQuality({ ...( { } as any), optimization } as any);
...
export function toPublicGuideContext(context: GuideContext) {
  return {
    ...
    optimization: (context as any).optimization ? { period:(context as any).optimization.period, project:(context as any).optimization.project, areas:(context as any).optimization.areas, dataQuality:(context as any).optimization.dataQuality, sourceHealth:(context as any).optimization.sourceHealth } : undefined,
