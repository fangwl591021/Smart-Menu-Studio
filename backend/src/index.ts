...
app.post('/api/crm/people/:personReference/assignment',async c=>{try{requireRole(c,'admin');const workspaceId=workspaceIdOf(c),person:any=await crmPersonByReference(c.env.smart_menu_db,{workspaceId,publicRef:c.req.param('personReference')});if(!person)return c.json({success:false,error:'NOT_FOUND'},404);const body:any=await c.req.json();const userRef=text(body.assignedUserReference,100);if(!userRef)return c.json({success:false,error:'ASSIGNED_USER_REQUIRED'},400);return c.json({success:true,result:await assignCrmOwner(c.env.smart_menu_db,{workspaceId,personId:person.id,assignedUserId:userRef,assignedByUserId:text(c.get('userId'))||null,reason:text(body.reason,500)})});}catch(e:any){return c.json({success:false,error:e?.message==='FORBIDDEN_ROLE'?'FORBIDDEN':e?.message==='CRM_ASSIGNED_USER_SCOPE_INVALID'?'ASSIGNED_USER_NOT_FOUND':'CRM_ASSIGNMENT_FAILED'},e?.message==='FORBIDDEN_ROLE'?403:400)}});
export default app;
