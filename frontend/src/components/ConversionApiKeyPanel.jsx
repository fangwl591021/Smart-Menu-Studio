...
export default function ConversionApiKeyPanel({ request, userRole }) {
  ...
  return <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm" aria-label="Conversion API">
    <div><h2 className="text-xl font-bold text-gray-900">Conversion API</h2><p className="mt-1 text-sm text-gray-500">Server-to-server conversion ingestion credentials. They are separate from LINE and user-session credentials.</p><details className="mt-3 rounded border border-gray-200 bg-gray-50 p-3 text-xs text-gray-700"><summary className="cursor-pointer font-bold">SourceCode contract / sample request</summary><p className="mt-2">sourceCode 可用：PURCHASE、BOOKING、SIGNUP、LIFF_REGISTRATION、CUSTOM；conversionType 仍是獨立欄位。Legacy 未記錄來源不會被回填。</p><pre className="mt-2 overflow-x-auto rounded bg-white p-2">{`POST /api/intelligence/conversions\nAuthorization: Bearer smc_live_&lt;prefix&gt;_&lt;secret&gt;\n{"sourceCode":"PURCHASE","conversionType":"purchase","externalEventId":"example-123","valueMinor":1000,"currency":"TWD"}`}</pre></details></div>
