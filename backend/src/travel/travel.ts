...
function itineraryView(row: any) {
...
function departureView(row: any) {
  ...
  return { safeDepartureReference: row.public_ref, safeItineraryReference: row.itinerary_ref, itineraryTitle: row.itinerary_title,
    ...
    bookingClosesAt: row.booking_closes_at, seatLimit: limit, reservedTravelerCount: reserved, remainingSeats: Math.max(0, limit - reserved),
...
const departureSelect = `SELECT d.*,i.public_ref itinerary_ref,i.title itinerary_title,i.status itinerary_status,i.seller_dealer_id,
...
export async function createItinerary(db: D1Database, input: any) {
  exact(input.body, ['title','summary','durationDays'], 'TRAVEL_ITINERARY_INPUT_INVALID');
  ...
  await db.batch([
    db.prepare(`INSERT INTO travel_itineraries(id,public_ref,workspace_id,title,summary,duration_days,status,created_by_user_id,created_at,updated_at) VALUES(?,?,?,?,?,?,'DRAFT',?,?,?)`)
      .bind(id, ref, input.workspaceId, title, clean(input.body.summary, 4000), integer(input.body.durationDays,1,365,'TRAVEL_DURATION_INVALID'), input.userId || null, timestamp, timestamp),
...
export async function listItineraries(db:D1Database,workspaceId:string){return (await rows(db.prepare(`SELECT * FROM travel_itineraries WHERE workspace_id=? ORDER BY updated_at DESC,id DESC`).bind(workspaceId))).map(itineraryView);}
export async function readItinerary(db:D1Database,workspaceId:string,reference:string){const row=await db.prepare(`SELECT * FROM travel_itineraries WHERE workspace_id=? AND public_ref=? LIMIT 1`).bind(workspaceId,reference).first<any>();if(!row)throw new Error('TRAVEL_ITINERARY_NOT_FOUND');return itineraryView(row);}
export async function updateItinerary(db:D1Database,input:any){
  ...
  await db.prepare(`UPDATE travel_itineraries SET title=?,summary=?,duration_days=?,status='DRAFT',review_note='',rejected_at=NULL,updated_at=? WHERE workspace_id=? AND id=?`)
    .bind(title,input.body.summary===undefined?row.summary:clean(input.body.summary,4000),input.body.durationDays===undefined?row.duration_days:integer(input.body.durationDays,1,365,'TRAVEL_DURATION_INVALID'),now(),input.workspaceId,row.id).run();
...
export async function createDeparture(db:D1Database,input:any){
  exact(input.body,['departureDate','returnDate','bookingOpensAt','bookingClosesAt','seatLimit','priceAmountMinor','currencyCode','paymentScheduleType','depositAmountMinor','depositDueAt','balanceDueAt'],'TRAVEL_DEPARTURE_INPUT_INVALID');
  ...
  const terms=schedule(input.body),id=makeId('trd'),ref=publicRef('dep'),timestamp=now();
  await db.batch([db.prepare(`INSERT INTO travel_departures(id,public_ref,workspace_id,itinerary_id,status,departure_date,return_date,booking_opens_at,booking_closes_at,seat_limit,price_amount_minor,currency_code,payment_schedule_type,deposit_amount_minor,deposit_due_at,balance_due_at,created_by_user_id,created_at,updated_at) VALUES(?,?,?,?,'DRAFT',?,?,?,?,?,?,'TWD',?,?,?,?,?,?,?)`)
    .bind(id,ref,input.workspaceId,itinerary.id,departureDate,returnDate,opens,closes,integer(input.body.seatLimit,1,10000,'TRAVEL_SEAT_LIMIT_INVALID'),terms.price,terms.type,terms.deposit,terms.depositDue,terms.balanceDue,input.userId||null,timestamp,timestamp),
...
function bookingView(row:any,travelers:any[]=[],schedules:any[]=[]){return {safeBookingReference:row.public_ref,safeOrderReference:row.order_ref,safeDepartureReference:row.departure_ref,safeItineraryReference:row.itinerary_ref,itineraryTitle:row.itinerary_title,departureDate:row.departure_date,bookingStatus:row.booking_status,travelerCount:Number(row.traveler_count),paymentScheduleType:row.payment_schedule_type_snapshot,totalAmountMinor:Number(row.total_amount_minor_snapshot),currencyCode:'TWD',sellerContext:row.seller_dealer_id?'DEALER':'TENANT',travelers:travelers.map(t=>({sequence:Number(t.sequence_no),displayName:t.display_name,travelerType:t.traveler_type,phone:t.phone,note:t.note})),paymentSchedule:schedules.map(scheduleView),createdAt:row.created_at,updatedAt:row.updated_at};}
const bookingSelect=`SELECT b.*,o.public_ref order_ref,d.public_ref departure_ref,d.departure_date,i.public_ref itinerary_ref,i.title itinerary_title FROM travel_booking_extensions b JOIN commerce_orders o ON o.workspace_id=b.workspace_id AND o.id=b.order_id JOIN travel_departures d ON d.workspace_id=b.workspace_id AND d.id=b.departure_id JOIN travel_itineraries i ON i.workspace_id=b.workspace_id AND i.id=d.itinerary_id`;
...
export async function listPublishedItineraries(db:D1Database,workspaceId:string){return (await rows(db.prepare(`SELECT * FROM travel_itineraries WHERE workspace_id=? AND status='PUBLISHED' ORDER BY published_at DESC,id DESC`).bind(workspaceId))).map(itineraryView);}
