...
export async function crmTimeline(db:D1Database,input:{workspaceId:string;personId:string;limit?:number;cursor?:string|null;viewer?:boolean}){
  ...
  const [profile,acquisition,referral,imports,cards,tags,insights,traits,stage,owners,followUps,points,rewards,contribution,tiers,commission,settlements,payouts,payments]=await Promise.all([
