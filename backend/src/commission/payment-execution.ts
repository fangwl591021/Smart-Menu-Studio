...
export function publicDealerPaymentStatusRow(row: Record<string, unknown>) { return { payoutRequestStatus: String(row.payout_request_status || ''), paymentStatus: row.payment_status || null, amountMinor: Number(row.amount_minor || 0), currencyCode: 'TWD', executionMode: row.payment_status ? 'SIMULATED' as const : null }; }
